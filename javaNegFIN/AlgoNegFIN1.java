...
public class AlgoNegFIN1 {
    ...
    private int numOfTrans,numOfTrans1; //// Number of transactions
    public int numOfFItem,numOfFItem1; // Number of items
    int outputCount = 0;// number of itemsets found
    public int minSupport,minSupport1; // minimum count
    public Item[] item,item1; // list of items sorted by count
    ...
    Item[] scanDB_Rank(String filename, double minSupport) throws IOException {
        numOfTrans1 = 0;
...
//        
//        for (Entry<Integer, Integer>i2 : mapItemCount.entrySet()){
...
//        }
        ...
        numOfFItem1 = mapItemCount.size();

        Item[] tempItems = new Item[numOfFItem1];
        int i = 0;
        for (Entry<Integer, Integer> entry : mapItemCount.entrySet()) {
            if (entry.getValue() >= this.minSupport1) {
...
//        for (int x=0; x< i;x++){
//        	System.out.println("item 131:"+tempItems[x].index);
//        }
...
//    	System.out.println("item:"+tempItems.length);
//    	System.out.println("======================================================");

//        for (int x=0; x< i;x++){
//        	System.out.println("item 131:"+item1[x].index);
//        }
//    	System.out.println("======================================================");
    	return item1;
    }
...
//
//        for (int x=0; x< i;x++){
//        	System.out.println("item 131:"+item[x].index);
//        }
    }
    ...
    void construct_BMC_tree(String filename) throws IOException {
        ...
        bmcTreeRoot.label = -1;
        bmcTreeRoot.bitmapCode = new MyBitVector1(numOfFItem);
...
//                        System.out.print(":"+itemX);
        ...
        while (root != null) {
            root.bitmapCode = (MyBitVector1) root.father.bitmapCode.clone();
...
//        nlLenSum += Math.pow(2.0, sameCount) * curNode.nodeset.size();
    ...
    public void runAlgorithm(String filename, double minsup, String output)
    	...
    	System.out.println("Algo Negin1:");
...
class MyBitVector1 {
    ...
    public MyBitVector1(int numOfBits) {
        bits = new long[((numOfBits - 1) / 64) + 1];
        cardinality = 0;
    }

    public MyBitVector1(int[] itemset, int last) {
        ...
        for (int i = 0; i < last; i++) {
            item = itemset[i];
            bits[item / 64] |= MyBitVector1.TWO_POWER[item % 64];
        }
    }

    public Object clone() {
        MyBitVector1 result = new MyBitVector1(this.bits.length * 64);
    ...
    public void set(int bitIndex) {
        bits[bitIndex / 64] |= MyBitVector1.TWO_POWER[bitIndex % 64];
    }

    public boolean isSet(int bitIndex) {
        return (bits[bitIndex / 64] & MyBitVector1.TWO_POWER[bitIndex % 64]) != 0;
    }

    public boolean isSubSet(MyBitVector1 q) {
